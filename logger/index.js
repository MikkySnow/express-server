...
let stackTrace = require('stack-trace')  // Для получения имени родительского модуля
let util = require('util') // util.inspect()
let path = require('path') // path.relative() path.sep
let projectname = require('../package').name // package.json -> project name

module.exports = class Logger { // Класс логера :)
  constructor () {
    function generateLogFunction (level) { // Функция генератор функций логгера :)
